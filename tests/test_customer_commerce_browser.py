...
def _catalog(prefix, *, customization=False, ready_designed=False):
    ...
    org = Organization.objects.create(
        ...
        display_name=f"{prefix} Browser Brand",
        email=f"{prefix}-browser@brand.test",
    ...
    artwork = Artwork.objects.create(
        organization=org,
        title=f"{prefix} Browser Artwork",
    ...
    designed = DesignedProduct.objects.create(
        ...
        title=f"{prefix} Browser Designed",
    ...
    store = create_storefront(
        organization=org,
        actor=owner,
        slug=f"{prefix}-browser-store",
        name_en=f"{prefix} Browser Store",
        name_ar=f"متجر {prefix} للمتصفح",
        about_en="Browser QA storefront backed by real test database rows.",
        about_ar="متجر اختبار متصفح يعتمد على سجلات قاعدة البيانات الفعلية للاختبار.",
    )
    ...
    product = create_store_product(
        ...
        slug=f"{prefix}-browser-product",
        title_en=f"{prefix} Browser Product",
        title_ar=f"منتج {prefix} للمتصفح",
        description_en="Customer commerce browser QA product.",
        description_ar="منتج لاختبار رحلة شراء العميل عبر المتصفح.",
        base_price="500.00",
    ...
    image = MediaAsset.objects.create(
        ...
        original_filename=f"{prefix}-browser.svg",
...
def _user_click(driver, by, locator):
    ...
    actions.scroll_to_element(element).move_to_element(element).pause(0.05).click().perform()
...
def _fill_checkout(driver):
    ...
    _user_click(driver, By.CSS_SELECTOR, 'label[data-payment-method="cod"]')
    assert driver.find_element(By.ID, "payment-cod").is_selected()
...
def test_real_chrome_mixed_customer_commerce_journey(client, live_server):
    ...
    try:
        ...
        wait.until(EC.presence_of_element_located((By.ID, "zone")))
        project = StudioProject.objects.get(customer=customer, product=custom)
        Select(driver.find_element(By.ID, "zone")).select_by_value(str(custom_zone.pk))
        driver.find_element(By.ID, "text").send_keys("FABINZI")
        _shot(driver, "02-studio-desktop-light.png")
        _user_click(driver, By.XPATH, '//form[input[@name="action" and @value="add_text"]]//button')
        wait.until(EC.presence_of_element_located((By.XPATH, '//*[contains(text(), "FABINZI")]')))
        _user_click(driver, By.XPATH, '//form[input[@name="action" and @value="ready"]]//button')
        wait.until(EC.presence_of_element_located((By.XPATH, '//form[input[@name="action" and @value="add_cart"]]//button')))
        _user_click(driver, By.XPATH, '//form[input[@name="action" and @value="add_cart"]]//button')
        wait.until(EC.url_contains(reverse("cart")))
        ...
        _user_click(driver, By.CSS_SELECTOR, 'form.checkout-layout button[type="submit"]')
        ...
        _user_click(driver, By.CSS_SELECTOR, f'a[href="{reverse("purchase-detail", args=[purchase.pk])}"]')
