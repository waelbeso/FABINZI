          ...
          return Card(
            child: ListTile(
              ...
              leading: CircleAvatar(
                child: Icon(
                  project.isReady ? Icons.check_rounded : Icons.edit_outlined,
                ),
              ),
              title: Text(
              ...
              subtitle: Text(
                '${project.isReady ? L10n.t(context, 'ready') : L10n.t(context, 'draft')} · ${project.unitPrice.display}',
              ),
...
class _StudioEditorScreenState extends State<StudioEditorScreen> {
  ...
  Future<void> load() async {
    ...
    try {
      final results = await Future.wait([
        ...
        widget.controller.api.product(
          project.product.storeSlug,
          project.product.productSlug,
        ),
      ]);
      if (!mounted) return;
      setState(() {
  ...
  Future<void> persistElement(StudioElement local) async {
    ...
    try {
      final saved = await widget.controller.api.updateStudioElement(
        project.id,
        local.id,
        transform: local.transform,
        productionMethod: local.productionMethod,
        text: local.kind == 'text' ? local.text : null,
      );
      if (!mounted) return;
      setState(() {
  ...
  void transformLocal(
    ...
    final movedX =
        (current.transform.x + details.focalPointDelta.dx / zoneSize.width)
            .clamp(0.0, 1.0);
    final movedY =
        (current.transform.y + details.focalPointDelta.dy / zoneSize.height)
            .clamp(0.0, 1.0);
    final scaled = (gestureBase!.scale * details.scale).clamp(.05, 1.0);
    final rotated = gestureBase!.rotation + details.rotation * 180 / math.pi;
    setState(
      () => elements[currentIndex] = current.copyWith(
              ...
              children: [
                DropdownButtonFormField<DecorationZone>(
                  ...
                  onChanged: (value) {
                    ...
                    setDialogState(() {
                      zone = value;
                      method = value.supportedMethods.first;
                    });
                  },
                ),
                ...
                if (allowed.isEmpty)
                  Padding(
                    ...
                    child: Text(L10n.t(context, 'unavailable')),
                  ),
              ],
            ),
            actions: [
              ...
              FilledButton(
                onPressed: allowed.isEmpty
                    ? null
                    : () =>
                          Navigator.pop(context, _ElementChoice(zone, method)),
                child: Text(L10n.t(context, 'continue')),
              ),
            ],
          );
        },
  ...
  Future<void> addText() async {
    ...
    final value = await showDialog<String>(
      ...
      builder: (context) => AlertDialog(
        ...
        content: TextField(
          ...
          autofocus: true,
          maxLength: 120,
          decoration: InputDecoration(labelText: L10n.t(context, 'text')),
        ),
        actions: [
          TextButton(
            onPressed: () => Navigator.pop(context),
    ...
    try {
      final element = await widget.controller.api.addStudioElement(
        project.id,
        zone: choice.zone.name,
        kind: 'text',
        text: value,
        productionMethod: choice.method,
  ...
  Future<void> addArtwork() async {
    ...
    try {
      final element = await widget.controller.api.addStudioElement(
        ...
        zone: choice.zone.name,
        kind: 'artwork',
        artworkVersionId: selected.approvedVersionId,
        productionMethod: choice.method,
        rightsConfirmed: false,
        sortOrder: elements.length,
      );
  ...
  Future<void> uploadImage() async {
    ...
    if (!(lower.endsWith('.png') ||
      ...
      if (mounted) {
        await showDialog<void>(
          context: context,
          builder: (context) => AlertDialog(
            content: Text(L10n.t(context, 'unsupportedImage')),
            actions: [
              TextButton(
                onPressed: () => Navigator.pop(context),
    ...
    if (bytes.length > 10485760) {
      if (mounted) {
        await showDialog<void>(
          context: context,
          builder: (context) => AlertDialog(
            content: Text(L10n.t(context, 'fileTooLarge')),
            actions: [
              TextButton(
                onPressed: () => Navigator.pop(context),
                child: Text(L10n.t(context, 'close')),
      ...
      builder: (context) => StatefulBuilder(
        builder: (context, setDialogState) => AlertDialog(
          ...
          content: CheckboxListTile(
            ...
            title: Text(L10n.t(context, 'rightsConfirm')),
          ),
          actions: [
            TextButton(
              onPressed: () => Navigator.pop(context),
    ...
    setState(() => saving = true);
    try {
      final upload = await widget.controller.api.uploadStudioImage(
        project.id,
        bytes,
        picked.name,
      );
    ...
    try {
      ...
      if (!validation.valid) {
        await showDialog<void>(
          context: context,
          builder: (context) => AlertDialog(
            title: Text(L10n.t(context, 'validationFailed')),
              ...
              return wide
                  ...
                  : Column(
                      children: [
                        ...
                        SizedBox(height: 250, child: tools),
                      ],
                    );
            },
          ),
      ...
      children: [
        Row(
          children: [
            Expanded(
              child: Text(
                project.isReady
                    ? L10n.t(context, 'ready')
                    : L10n.t(context, 'draft'),
                style: Theme.of(context).textTheme.titleMedium
                    ?.copyWith(fontWeight: FontWeight.w900),
              ),
            ),
            MoneyText(project.unitPrice),
          ],
        ),
        const SizedBox(height: 12),
        Row(
        ...
        if (project.isDraft) ...[
          Wrap(
            ...
            children: [
              FilledButton.tonalIcon(
                onPressed: saving ? null : addText,
                icon: const Icon(Icons.text_fields),
                label: Text(L10n.t(context, 'addText')),
              ),
              FilledButton.tonalIcon(
                onPressed: saving ? null : addArtwork,
                icon: const Icon(Icons.palette_outlined),
                label: Text(L10n.t(context, 'addArtwork')),
              ),
              FilledButton.tonalIcon(
                onPressed: saving ? null : uploadImage,
                icon: const Icon(Icons.add_photo_alternate_outlined),
                label: Text(L10n.t(context, 'uploadImage')),
              ),
            ],
          ),
          if (selectedElementId != null) ...[
            ...
            OutlinedButton.icon(
              onPressed: saving ? null : deleteSelected,
              icon: const Icon(Icons.delete_outline),
              label: Text(L10n.t(context, 'delete')),
            ),
          ],
          const Divider(height: 28),
          FilledButton.icon(
            onPressed: saving ? null : validateAndReady,
            icon: const Icon(Icons.verified_outlined),
            label: Text(L10n.t(context, 'markReady')),
          ),
        ] else ...[
          Text(L10n.t(context, 'readyForCart')),
          const SizedBox(height: 12),
          FilledButton.icon(
            onPressed: saving ? null : addReadyToCart,
            icon: const Icon(Icons.shopping_bag_outlined),
            label: Text(L10n.t(context, 'addToCart')),
          ),
          const SizedBox(height: 8),
          OutlinedButton.icon(
            onPressed: saving ? null : directCheckout,
            icon: const Icon(Icons.payment_outlined),
            label: Text(L10n.t(context, 'checkout')),
          ),
        ],
      ],
    ),
...
class _StudioCanvas extends StatelessWidget {
              ...
              return GestureDetector(
                ...
                child: Stack(
                  children: [
                    ...
                    ...project.decorationZones.map(
                      (zone) => _zone(context, zone, size),
                    ),
                  ],
                ),
              );
  ...
  Widget _zone(BuildContext context, DecorationZone zone, Size canvas) {
    final rect = Rect.fromLTWH(
      zone.placement.x * canvas.width,
      zone.placement.y * canvas.height,
      zone.placement.width * canvas.width,
      zone.placement.height * canvas.height,
    );
    final zoneElements = elements
        .where((element) => element.decorationZone.name == zone.name)
        .toList();
    return Positioned(
      left: rect.left,
      top: rect.top,
        ...
        builder: (context, constraints) {
          ...
          return Stack(
            ...
            children: [
              ...
              ...zoneElements.map(
                (element) => _element(context, element, zoneSize),
              ),
            ],
          );
        },
  ...
  Widget _element(BuildContext context, StudioElement element, Size zoneSize) {
    final extent =
        element.transform.scale * math.min(zoneSize.width, zoneSize.height);
    ...
    final display = switch (element.kind) {
      'text' => Center(
        child: Text(
          ...
          maxLines: 3,
          style: TextStyle(
            fontSize: math.max(10, extent * .18),
            fontWeight: FontWeight.w800,
          ),
        ),
      ),
      'image' when element.sourceUrl != null => ProtectedImage(
      ...
      child: Transform.rotate(
        ...
        child: GestureDetector(
          ...
          onScaleUpdate: (details) =>
              onGestureUpdate(element, details, zoneSize),
          onScaleEnd: (_) {
