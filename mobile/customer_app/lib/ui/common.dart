...
class MoneyText extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) => Text(money.display, style: style ?? Theme.of(context).textTheme.titleMedium?.copyWith(fontWeight: FontWeight.w800));
}

class PublicImage extends StatelessWidget {
  const PublicImage({super.key, required this.image, this.height = 190, this.fit = BoxFit.cover});
  ...
  Widget build(BuildContext context) {
    ...
    return Semantics(
      ...
      child: Image.network(
        ...
        fit: fit,
        errorBuilder: (_, __, ___) => Container(height: height, color: Theme.of(context).colorScheme.surfaceContainerHighest, child: const Center(child: Icon(Icons.broken_image_outlined, size: 42))),
        loadingBuilder: (context, child, progress) => progress == null ? child : SizedBox(height: height, child: const Center(child: CircularProgressIndicator())),
      ),
    );
  }
}
...
class ProtectedImage extends StatefulWidget {
  const ProtectedImage({super.key, required this.controller, required this.url, this.fit = BoxFit.contain});
