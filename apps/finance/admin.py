...
class FinancePolicyAdmin(admin.ModelAdmin):
    list_display = ("name", "platform_fee_bps", "settlement_delay_days", "minimum_payout", "is_active", "updated_at")


@admin.register(PayoutProfile, site=fabinzi_admin_site)
class PayoutProfileAdmin(admin.ModelAdmin):
    list_display = ("organization", "method", "status", "account_holder", "destination_hint", "updated_at")
...
class OrderFinanceAdmin(admin.ModelAdmin):
    list_display = ("order", "gross_amount", "platform_fee", "manufacturer_payable", "designer_earnings", "currency", "recognized_at", "available_at")
    list_filter = ("currency", "status")
    readonly_fields = ("order", "designer_account", "manufacturer_account", "gross_amount", "platform_fee", "manufacturer_payable", "designer_earnings", "currency", "policy_snapshot", "recognized_at", "available_at")
...
class LedgerEntryAdmin(admin.ModelAdmin):
    ...
    readonly_fields = ("account", "order_finance", "settlement", "entry_type", "amount", "currency", "available_at", "memo", "created_by", "created_at")
...
class SettlementRequestAdmin(admin.ModelAdmin):
    list_display = ("id", "organization", "amount", "currency", "status", "requested_at", "paid_at")
    list_filter = ("status", "currency")
    search_fields = ("organization__display_name", "external_reference")
