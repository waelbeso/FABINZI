...
def _membership(actor,organization,roles):
    ...
    m=Membership.objects.filter(organization=organization,user=actor,is_active=True).first()
    if not m or m.role not in roles: raise PermissionDenied("Business role does not permit this operation.")
    return m
def require_designer_operations(actor,order): _membership(actor,order.designer_organization,DESIGNER_OP_ROLES)
...
def start_order_operations(*,order,actor=None,request=None):
    ...
    f,created=FulfillmentRecord.objects.get_or_create(order=order,defaults={"status":FulfillmentRecord.Status.READY_TO_PACK if stock else FulfillmentRecord.Status.WAITING_PRODUCTION})
    if created: _event(f,f.status,actor,"Operations initialized")
    job=None
    if not stock:
        job,jc=ProductionJob.objects.get_or_create(order=order)
        if jc:
...
def assign_manufacturer(*,job,selection,actor,request=None):
    job=ProductionJob.objects.select_for_update().get(pk=job.pk); require_designer_operations(actor,job.order)
...
def record_qc(*,job,actor,decision,checklist=None,notes="",request=None):
    ...
    q=QCInspection.objects.create(job=job,decision=decision,checklist=checklist or {},notes=notes,inspected_by=actor)
    if decision==QCInspection.Decision.PASSED:
        job.status=ProductionJob.Status.READY; job.ready_at=timezone.now(); job.save(update_fields=["status","ready_at","updated_at"]); f=job.order.fulfillment; f.status=FulfillmentRecord.Status.READY_TO_PACK; f.save(update_fields=["status","updated_at"]); _event(f,f.status,actor,"QC passed")
    else: job.status=ProductionJob.Status.QC_FAILED; job.save(update_fields=["status","updated_at"])
    record_audit_event(actor=actor,action="production_job.qc_recorded",instance=job,metadata={"decision":decision},request=request); return q
@transaction.atomic
def add_production_asset(*,job,actor,media_asset,kind,label="",request=None):
    ...
    a=ProductionAsset(job=job,media_asset=media_asset,kind=kind,label=label,uploaded_by=actor); a.full_clean(); a.save(); record_audit_event(actor=actor,action="production_asset.added",instance=a,request=request); return a
def _require_fulfillment_actor(actor,f):
    order=f.order
...
def pack_order(*,fulfillment,actor,request=None):
    f=FulfillmentRecord.objects.select_for_update().select_related("order").get(pk=fulfillment.pk); _require_fulfillment_actor(actor,f)
    if f.status!=FulfillmentRecord.Status.READY_TO_PACK: raise ValidationError("Order is not ready to pack.")
    f.status=FulfillmentRecord.Status.PACKED; f.packed_at=timezone.now(); f.save(update_fields=["status","packed_at","updated_at"]); _event(f,f.status,actor); record_audit_event(actor=actor,action="fulfillment.packed",instance=f,request=request); return f
@transaction.atomic
def ship_order(*,fulfillment,actor,carrier,tracking_number,tracking_url="",request=None):
    f=FulfillmentRecord.objects.select_for_update().select_related("order").get(pk=fulfillment.pk); _require_fulfillment_actor(actor,f)
    if f.status!=FulfillmentRecord.Status.PACKED: raise ValidationError("Order must be packed before shipping.")
    if not carrier.strip() or not tracking_number.strip(): raise ValidationError("Carrier and tracking number are required.")
    f.status=FulfillmentRecord.Status.SHIPPED; f.carrier=carrier.strip(); f.tracking_number=tracking_number.strip(); f.tracking_url=tracking_url; f.shipped_at=timezone.now(); f.save(); _event(f,f.status,actor,f"{f.carrier} · {f.tracking_number}"); _notify_customer(f.order,"Order shipped","تم شحن الطلب",f"Order {f.order.number} has shipped.",f"تم شحن الطلب {f.order.number}."); record_audit_event(actor=actor,action="fulfillment.shipped",instance=f,metadata={"carrier":carrier},request=request); return f
@transaction.atomic
def deliver_order(*,fulfillment,actor,request=None):
    f=FulfillmentRecord.objects.select_for_update().select_related("order").get(pk=fulfillment.pk); _require_fulfillment_actor(actor,f)
    if f.status!=FulfillmentRecord.Status.SHIPPED: raise ValidationError("Only shipped orders can be delivered.")
    f.status=FulfillmentRecord.Status.DELIVERED; f.delivered_at=timezone.now(); f.save(update_fields=["status","delivered_at","updated_at"]); _event(f,f.status,actor); _notify_customer(f.order,"Order delivered","تم تسليم الطلب",f"Order {f.order.number} was delivered.",f"تم تسليم الطلب {f.order.number}."); record_audit_event(actor=actor,action="fulfillment.delivered",instance=f,request=request)
    from apps.finance.services import recognize_order_finance
    recognize_order_finance(order=f.order,actor=actor,request=request)
    return f