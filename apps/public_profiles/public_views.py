...
def _location_parts(organization):
    parts = []
    seen = set()
    for value in (organization.city, organization.region, organization.country):
...
def _dedupe_canonical_display(rows):
    result = []
    seen = set()
    for row in rows:
...
def manufacturer_directory(request):
    manufacturers = list(
        ...
        .select_related("public_state__profile_image", "marketplace_listing")
    ...
    for index, organization in enumerate(manufacturers, 1):
        ...
        headline = ""
        if listing:
            headline = listing.headline_ar if getattr(request, "LANGUAGE_CODE", "en") == "ar" and listing.headline_ar else listing.headline_en
        url = reverse("manufacturer-public-detail", args=[state.slug])
        cards.append({
        ...
        items.append({"@type": "ListItem", "position": index, "name": name, "url": absolute_url(url)})
    return render(request, "public_profiles/manufacturer_directory.html", {
        "manufacturers": manufacturers,
        "manufacturer_cards": cards,
        "page_seo": page_seo(
...
def manufacturer_public_detail(request, slug):
    organization = get_object_or_404(
        public_professional_queryset(kind=Organization.Kind.MANUFACTURER).select_related(
    ...
    state = organization.public_state
    listing = getattr(organization, "marketplace_listing", None)
    capabilities = _dedupe_canonical_display(list(verified_canonical_capabilities(organization)))
