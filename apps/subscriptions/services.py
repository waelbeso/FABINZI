...
def _local_date(value=None):
    return timezone.localtime(_now(value)).date()
...
def get_effective_plan(code, *, at=None):
    day = _local_date(at)
    plan = (
        SubscriptionPlanPolicy.objects.filter(code=code, active=True, effective_from__lte=day)
...
def _period_end(start, *, hard_end=None):
    ...
    return hard_end if hard_end and end > hard_end else end
...
def ensure_subscription_for_organization(organization, *, activation_at=None, actor=None, request=None):
    """Idempotently provision entitlement state at an explicit professional activation boundary."""
    ...
    if organization.kind == Organization.Kind.MANUFACTURER:
        ...
        subscription = OrganizationSubscription.objects.create(
            ...
            current_period_end=_period_end(started_at, hard_end=trial_ends_at),
...
def _subscription_locked_for_org(organization, *, now=None):
    ...
    subscription = (
        ...
        .select_related("current_plan", "organization")
...
def _design_usage(organization):
    return (
        GarmentDesign.objects.filter(organization=organization, status__in=DESIGN_SLOT_STATUSES)
        .exclude(plan_entitlement_state__plan_paused=True)
        .count()
    )
...
def _artwork_usage(organization):
    return (
        Artwork.objects.filter(organization=organization, status__in=ARTWORK_SLOT_STATUSES)
        .exclude(plan_entitlement_state__plan_paused=True)
        .count()
    )
        ...
        summary.update(
            {
                "offer_limit": limit,
                "offer_used": used,
                "offer_remaining": max(0, limit - used),
...
def assert_designer_slot_available(*, organization, kind, object_id=None, now=None):
    ...
    if qs.count() >= limit:
        raise ValidationError(
            f"The current plan allows {limit} active {kind} slot(s). Archive or pause an active item, or upgrade the plan."
        )
    return True
...
def consume_manufacturer_offer(*, quote, now=None):
    ...
    existing = ManufacturerOfferUsage.objects.filter(quote=quote).first()
    if existing:
        return existing
    subscription = _subscription_locked_for_org(organization, now=now)
    snapshot = dict(subscription.policy_snapshot or {})
    ...
    if used >= limit:
        raise ValidationError(
            f"The current plan allows {limit} submitted Manufacturing Offer(s) in this monthly usage period."
        )
    try:
...
def create_team_invitation(*, organization, actor, email, role, request=None, now=None):
    ...
    invitation = TeamInvitation.objects.create(
        ...
        created_by=actor,
        expires_at=now + timedelta(days=TeamInvitationConfiguration.current_expiry_days()),
    )
...
def accept_team_invitation(*, token, actor, request=None, now=None):
    ...
    record_audit_event(
        ...
        instance=membership,
        metadata={"organization_id": organization.pk, "invitation_id": invitation.pk, "role": membership.role},
        request=request,
    )
    return membership
...
def suspend_team_member(*, membership, actor, request=None, reason="owner_action"):
    ...
    record_audit_event(
        ...
        instance=membership,
        metadata={"organization_id": membership.organization_id, "reason": reason},
        request=request,
    )
    return membership
...
def restore_team_member(*, membership, actor, request=None):
    ...
    membership.full_clean()
    membership.save(update_fields=["is_active"])
    MembershipPlanSuspension.objects.filter(membership=membership).update(
...
def change_team_member_role(*, membership, actor, role, request=None):
    ...
    record_audit_event(
        ...
        instance=membership,
        metadata={"organization_id": membership.organization_id, "role": role},
        request=request,
    )
    return membership
...
def _restore_store_product_pauses(organization, *, now=None):
    now = _now(now)
    pauses = StoreProductPlanPause.objects.filter(
        store_product__storefront__organization=organization,
        active=True,
...
def apply_designer_downgrade(
    *,
    ...
    active_designs = list(
        GarmentDesign.objects.select_for_update()
        .filter(organization=organization, status__in=DESIGN_SLOT_STATUSES)
        .order_by("created_at", "id")
    )
    active_artworks = list(
        Artwork.objects.select_for_update()
        .filter(organization=organization, status__in=ARTWORK_SLOT_STATUSES)
        .order_by("created_at", "id")
    )
    selected_designs = [row.pk for row in active_designs[:design_limit]] if retained_design_ids is None else [int(value) for value in retained_design_ids]
    selected_artworks = [row.pk for row in active_artworks[:artwork_limit]] if retained_artwork_ids is None else [int(value) for value in retained_artwork_ids]
    if len(set(selected_designs)) > design_limit or not set(selected_designs).issubset({row.pk for row in active_designs}):
        raise ValidationError("Designer retained Design selection exceeds Starter capacity or includes invalid content.")
    if len(set(selected_artworks)) > artwork_limit or not set(selected_artworks).issubset({row.pk for row in active_artworks}):
        raise ValidationError("Designer retained Artwork selection exceeds Starter capacity or includes invalid content.")
    selected_design_set = set(selected_designs)
    selected_artwork_set = set(selected_artworks)
    for design in active_designs:
        retained = design.pk in selected_design_set
        ...
        if not retained:
            ...
            record_audit_event(
                actor=actor,
                action="subscription.designer_content_plan_paused",
                instance=design,
                metadata={"organization_id": organization.pk, "protected_active_chain": protected},
                request=request,
            )
    for artwork in active_artworks:
        retained = artwork.pk in selected_artwork_set
    ...
    record_audit_event(
        actor=actor,
        action="subscription.designer_excess_selection",
        instance=organization,
        metadata={"retained_design_ids": selected_designs, "retained_artwork_ids": selected_artworks},
        request=request,
    )
...
def apply_designer_team_downgrade(
    *,
    ...
    if len(set(retained)) > limit or not set(retained).issubset(valid):
        raise ValidationError("Retained Designer team selection exceeds Starter seat capacity or includes invalid members.")
...
@transaction.atomic
def apply_manufacturer_team_downgrade(
    *,
    organization,
    actor=None,
    retained_membership_ids=None,
    ...
    limit = int(starter.team_subaccount_limit or 0)
    members = _team_members_for_downgrade(organization)
    retained = [membership.pk for membership in members[:limit]] if retained_membership_ids is None else [int(value) for value in retained_membership_ids]
    if len(set(retained)) > limit or not set(retained).issubset({membership.pk for membership in members}):
    ...
    for membership in members:
        ...
        record_audit_event(
            ...
            action="subscription.team_member_plan_suspended",
            instance=membership,
            metadata={"organization_id": organization.pk, "audience": "manufacturer"},
            request=request,
        )
    return suspended
...
def restore_plan_limited_resources(*, organization, actor=None, request=None, now=None):
    now = _now(now)
    if organization.kind == Organization.Kind.DESIGNER:
        DesignPlanEntitlementState.objects.filter(design__organization=organization, plan_paused=True).update(
            plan_paused=False,
            retained=False,
        ...
        ArtworkPlanEntitlementState.objects.filter(artwork__organization=organization, plan_paused=True).update(
            plan_paused=False,
            retained=False,
            protected_active_chain=False,
            pause_reason="",
...
def downgrade_to_starter(
    *,
    ...
    if organization.kind == Organization.Kind.DESIGNER:
        ...
        apply_manufacturer_team_downgrade(
            organization=organization,
            actor=actor,
            retained_membership_ids=retained_membership_ids,
            request=request,
    ...
    record_audit_event(
        ...
        action=action,
        instance=subscription,
        metadata={"organization_id": organization.pk, "old_plan": old_plan, "new_plan": starter.code},
        request=request,
    )
    _notify_subscription_owners(
        subscription,
        "subscription_downgraded",
        "Subscription moved to Starter",
        "تم نقل الاشتراك إلى Starter",
    )
    return subscription
...
def _lock_confirmation(pk):
    # Lock the evidence row only. PostgreSQL cannot SELECT FOR UPDATE the nullable
    # side of the consumed-period outer join.
    return (
...
def confirm_subscription_billing(
    *,
    ...
    now = _now(now)
    plan = get_effective_plan(plan_code, at=now)
    if plan.audience != organization.kind:
    ...
    expected_policy_snapshot = plan_snapshot(plan)
    expected_price_snapshot = price_snapshot(plan)
    defaults = {
        "organization": organization,
        "plan_policy": plan,
        "plan_code": plan.code,
...
def activate_paid_pro(*, organization, actor, billing_confirmation, request=None, now=None):
    ...
    subscription = _subscription_locked_for_org(organization, now=now)
    if _existing_consumed_period(confirmation, subscription):
        return subscription

    pro = get_effective_plan(pro_code_for(organization), at=now)
    _validate_confirmation_for_policy(confirmation=confirmation, organization=organization, plan=pro)
    old_plan = subscription.current_plan.code
    ...
    subscription.save()
    period = _create_period(subscription, billing_reference=confirmation.provider_reference)
...
def renew_paid_subscription(*, subscription, actor, billing_confirmation, request=None, now=None):
    ...
    effective_plan = get_effective_plan(subscription.current_plan.code, at=start)
    ...
    subscription.grace_started_on = None
    subscription.grace_ends_on = None
    subscription.policy_snapshot = plan_snapshot(effective_plan)
    subscription.price_snapshot = price_snapshot(effective_plan)
    subscription.save()
    period = _create_period(subscription, billing_reference=confirmation.provider_reference)
    _consume_confirmation(confirmation=confirmation, period=period, now=now)
    record_audit_event(
...
def _notify_subscription_owners(subscription, notification_type, title_en, title_ar, body_en="", body_ar=""):
    notifications = []
    for membership in subscription.organization.memberships.filter(
        role=Membership.Role.OWNER,
        is_active=True,
    ).select_related("user"):
        notifications.append(
            Notification.objects.create(
                ...
                body_en=body_en,
                body_ar=body_ar,
                destination="/designer/subscription/"
                if subscription.organization.kind == Organization.Kind.DESIGNER
                else "/manufacturer/subscription/",
            )
        )
...
def generate_due_reminders(*, subscription, now=None):
    ...
    for milestone in SubscriptionReminderMilestone.objects.filter(active=True):
        if today != due + timedelta(days=milestone.offset_days):
            continue
        event, is_new = SubscriptionReminderEvent.objects.get_or_create(
            subscription=subscription,
            due_date=due,
            milestone=milestone,
        )
    ...
    if subscription.status == OrganizationSubscription.Status.ACTIVE and today >= due_date:
        ...
        record_audit_event(
            ...
            metadata={
                ...
                "grace_ends_on": subscription.grace_ends_on.isoformat(),
            },
        )
        _notify_subscription_owners(
            subscription,
