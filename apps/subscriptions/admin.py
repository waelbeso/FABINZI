...
class SubscriptionPlanPolicyAdmin(admin.ModelAdmin):
    ...
    def save_model(self, request, obj, form, change):
        ...
        record_audit_event(actor=request.user, action="subscription.plan_configuration_changed" if change else "subscription.plan_configuration_created", instance=obj, metadata={"code": obj.code, "version": obj.version, "effective_from": obj.effective_from.isoformat()}, request=request)


@admin.register(OnboardingPlanSelection, site=fabinzi_admin_site)
class OnboardingPlanSelectionAdmin(admin.ModelAdmin):
    list_display = ("application", "organization_name", "plan_code", "plan_version", "selected_at", "payment_due_at")
    list_filter = ("plan_code", "plan_version", "application__status", "application__organization__kind")
    search_fields = ("application__organization__display_name", "application__organization__email", "plan_code")
    readonly_fields = tuple(field.name for field in OnboardingPlanSelection._meta.fields)

    @admin.display(description="Organization")
    def organization_name(self, obj):
        return obj.application.organization.display_name
...
class TeamInvitationConfigurationAdmin(admin.ModelAdmin):
    ...
    def has_add_permission(self, request): return not TeamInvitationConfiguration.objects.exists() and super().has_add_permission(request)
    def has_delete_permission(self, request, obj=None): return False
    def save_model(self, request, obj, form, change):
        obj.singleton_key = 1; obj.updated_by = request.user; super().save_model(request, obj, form, change)
...
class SubscriptionBillingConfirmationAdmin(admin.ModelAdmin):
    list_display = ("organization", "plan_code", "plan_version", "amount", "currency", "provider", "provider_reference", "status", "confirmed_at", "confirmed_by")
    ...
    def has_add_permission(self, request):
        try: require_subscription_operator(request.user)
        except PermissionDenied: return False
        return True
    def has_change_permission(self, request, obj=None): return False
    def has_delete_permission(self, request, obj=None): return False
    def save_model(self, request, obj, form, change):
        if change: raise PermissionDenied("Billing confirmation evidence is immutable.")
        confirmation = confirm_subscription_billing(organization=obj.organization, actor=request.user, plan_code=obj.plan_code, amount=obj.amount, currency=obj.currency, provider=obj.provider, provider_reference=obj.provider_reference, idempotency_key=obj.idempotency_key, request=request)
...
class OrganizationSubscriptionAdmin(admin.ModelAdmin):
    ...
    readonly_fields = ("organization", "current_plan", "status", "started_at", "trial_started_at", "trial_ends_at", "trial_consumed", "current_period_start", "current_period_end", "next_billing_at", "grace_started_on", "grace_ends_on", "cancelled_at", "downgraded_at", "policy_snapshot", "price_snapshot", "updated_at", "usage_summary")
    ...
    def usage_summary(self, obj):
        ...
        if obj.organization.kind == "designer": return f"Designs {summary['design_used']}/{summary['design_limit']} · Artworks {summary['artwork_used']}/{summary['artwork_limit']} · Team {summary['team_used']}/{summary['team_limit']}"
        return f"Offers {summary['offer_used']}/{summary['offer_limit']} · Team {summary['team_used']}/{summary['team_limit']}"
    def get_urls(self):
        custom = [path("<int:object_id>/activate-pro/", self.admin_site.admin_view(self.activate_pro_view), name="subscriptions_activate_pro"), path("<int:object_id>/downgrade-starter/", self.admin_site.admin_view(self.downgrade_view), name="subscriptions_downgrade_starter"), path("<int:object_id>/trial-exception/", self.admin_site.admin_view(self.trial_exception_view), name="subscriptions_trial_exception")]
        return custom + super().get_urls()
    def _obj(self, request, object_id):
        obj = self.get_object(request, object_id)
        if not obj: raise ValidationError("Subscription not found.")
        require_subscription_operator(request.user); return obj
    ...
    def downgrade_view(self, request, object_id):
        ...
        except (ValidationError, PermissionDenied) as exc: self.message_user(request, str(exc), messages.ERROR)
        else: self.message_user(request, "Subscription moved to Starter without deleting business records.", messages.SUCCESS)
        return HttpResponseRedirect(reverse("fabinzi_admin:subscriptions_organizationsubscription_change", args=[object_id]))
    def trial_exception_view(self, request, object_id):
        try:
            obj = self.get_object(request, object_id)
            if request.method != "POST": raise ValidationError("Lifecycle actions require POST.")
            grant_manufacturer_trial_exception(subscription=obj, actor=request.user, reason=request.POST.get("reason", "").strip(), request=request)
        except (ValidationError, PermissionDenied) as exc: self.message_user(request, str(exc), messages.ERROR)
        else: self.message_user(request, "Historical trial exception granted and audited.", messages.SUCCESS)
        return HttpResponseRedirect(reverse("fabinzi_admin:subscriptions_organizationsubscription_change", args=[object_id]))
