...
from .services import add_artwork_asset, add_ip_case_evidence, add_product_placement, create_artwork, create_artwork_revision, create_designed_product, create_ip_case, moderate_ip_case, publish_designed_product, require_artwork_draft, review_artwork_version, set_ip_declaration, submit_artwork_version, user_can_view_artwork
...
class ArtworkListCreateAPIView(APIView):
    ...
    def post(self, request):
        ...
        artwork = create_artwork(organization=org, actor=request.user, title=request.data.get("title", "").strip(), description=request.data.get("description", ""), tags=request.data.get("tags", []), request=request)
...
class ArtworkPublicAPIView(APIView):
    ...
    def get(self, request):
        qs = Artwork.objects.filter(status=Artwork.Status.APPROVED)
        return Response(ArtworkSerializer(qs, many=True).data)
...
class ArtworkDetailAPIView(APIView):
    def get(self, request, pk):
        ...
        if not user_can_view_artwork(request.user, artwork): return Response(status=403)
        return Response(ArtworkSerializer(artwork).data)


class ArtworkVersionDetailAPIView(APIView):
    def patch(self, request, version_id):
        version = get_object_or_404(ArtworkVersion, pk=version_id); require_artwork_draft(version, request.user)
        serializer = ArtworkVersionSerializer(version, data=request.data, partial=True); serializer.is_valid(raise_exception=True); serializer.save()
...
class ArtworkAssetAPIView(APIView):
    def post(self, request, version_id):
        asset = add_artwork_asset(version=get_object_or_404(ArtworkVersion, pk=version_id), actor=request.user, media_asset=get_object_or_404(MediaAsset, pk=request.data.get("media_asset")), kind=request.data.get("kind"), label=request.data.get("label", ""), request=request)
...
class IPDeclarationAPIView(APIView):
    def post(self, request, version_id):
        declaration = set_ip_declaration(version=get_object_or_404(ArtworkVersion, pk=version_id), actor=request.user, rights_basis=request.data.get("rights_basis"), rights_holder_name=request.data.get("rights_holder_name", ""), third_party_content=bool(request.data.get("third_party_content", False)), details=request.data.get("details", ""), accepts_ip_policy=bool(request.data.get("accepts_ip_policy", False)), request=request)
...
class ArtworkReviewAPIView(APIView):
    def post(self, request, version_id):
        version = review_artwork_version(version=get_object_or_404(ArtworkVersion, pk=version_id), reviewer=request.user, decision=request.data.get("decision"), notes=request.data.get("notes", ""), request=request)
        return Response(ArtworkVersionSerializer(version).data)
...
class DesignedProductListCreateAPIView(APIView):
    ...
    def post(self, request):
        product = create_designed_product(organization=get_object_or_404(Organization, pk=request.data.get("organization")), actor=request.user, garment_version=get_object_or_404(GarmentDesignVersion, pk=request.data.get("garment_version")), artwork_version=get_object_or_404(ArtworkVersion, pk=request.data.get("artwork_version")), title=request.data.get("title", "").strip(), description=request.data.get("description", ""), request=request)
        return Response(DesignedProductSerializer(product).data, status=201)


class ProductPlacementAPIView(APIView):
    def post(self, request, product_id):
        placement = add_product_placement(product=get_object_or_404(DesignedProduct, pk=product_id), actor=request.user, decoration_zone=get_object_or_404(DecorationZone, pk=request.data.get("decoration_zone")), transform=request.data.get("transform", {}), production_method=request.data.get("production_method"), request=request)
...
class IPCaseCreateAPIView(APIView):
    ...
    def post(self, request):
        ...
        case = create_ip_case(actor=request.user if request.user.is_authenticated else None, artwork=artwork, designed_product=product, reporter_name=request.data.get("reporter_name", "").strip(), reporter_email=request.data.get("reporter_email", "").strip(), claimant_rights=request.data.get("claimant_rights", ""), allegation=request.data.get("allegation", ""), request=request)
        return Response({"id":case.id,"status":case.status}, status=201)


class IPCaseEvidenceAPIView(APIView):
    def post(self, request, case_id):
        evidence = add_ip_case_evidence(case=get_object_or_404(IPCase, pk=case_id), actor=request.user, media_asset=get_object_or_404(MediaAsset, pk=request.data.get("media_asset")), description=request.data.get("description", ""), request=request)
        return Response({"id":evidence.id}, status=201)


class IPCaseModerateAPIView(APIView):
    def post(self, request, case_id):
        case = moderate_ip_case(case=get_object_or_404(IPCase, pk=case_id), reviewer=request.user, status=request.data.get("status"), resolution=request.data.get("resolution", IPCase.Resolution.NONE), notes=request.data.get("notes", ""), request=request)
