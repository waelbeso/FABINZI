...
class PublicInquiry(models.Model):
    ...
    designer_work_kind = models.CharField(
        max_length=24,
        choices=DesignerWorkKind.choices,
        blank=True,
    )
    garment_design = models.ForeignKey(
    ...
    class Meta:
        indexes = [
            models.Index(
                fields=["target_organization", "status", "created_at"],
                name="pubinq_target_status_idx",
            ),
            models.Index(
                fields=["sender_user", "created_at"],
                name="pubinq_sender_created_idx",
            ),
        ]
    ...
    def clean(self):
        ...
        if self.target_organization_id:
            expected = (
                ...
                else Organization.Kind.MANUFACTURER
            )
            if self.target_organization.kind != expected:
                raise ValidationError(
                    {"target_organization": "Inquiry target kind does not match the professional organization."}
                )
        selected = [self.garment_design_id, self.artwork_id, self.ready_product_id]
        if self.target_kind == self.TargetKind.DESIGNER:
            if self.manufacturer_product_approval_id:
                raise ValidationError(
                    "Designer inquiries cannot reference a Manufacturer public-product approval."
                )
            if sum(bool(value) for value in selected) > 1:
                raise ValidationError("A Designer inquiry may reference at most one public work.")
        elif self.target_kind == self.TargetKind.MANUFACTURER:
            if any(selected) or self.designer_work_kind:
                raise ValidationError(
                    "Manufacturer inquiries cannot reference Designer work fields."
                )
            if not self.manufacturer_product_approval_id:
                raise ValidationError(
                    "Manufacturer inquiries require an explicitly approved public product relationship."
                )
            if self.manufacturer_product_approval.manufacturer_id != self.target_organization_id:
                raise ValidationError(
                    "The selected public product is not approved for this Manufacturer."
                )
...
class PublicInquiryMessage(models.Model):
    ...
    actor = models.ForeignKey(
        ...
        blank=True,
        on_delete=models.SET_NULL,
        related_name="public_inquiry_messages",
    )
...
class PublicInquiryAttachment(models.Model):
    ...
    media_asset = models.ForeignKey(
        "media.MediaAsset",
        on_delete=models.PROTECT,
        related_name="public_inquiry_attachments",
    )
    ...
    def clean(self):
        ...
        if self.media_asset_id and self.media_asset.access != MediaAsset.Access.PRIVATE:
            raise ValidationError(
                {"media_asset": "Public inquiry attachments must remain PRIVATE MediaAssets."}
            )
