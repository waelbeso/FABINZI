...
def create_remote_payment(attempt, *, return_url=""):
    ...
    if attempt.provider == IntegrationConfig.Provider.STRIPE:
        ...
        response = requests.post("https://api.stripe.com/v1/payment_intents", auth=(secret, ""), data={"amount": int(attempt.amount * 100), "currency": attempt.currency.lower(), "metadata[order_number]": str(attempt.order.number), "automatic_payment_methods[enabled]": "true"}, headers={"Idempotency-Key": attempt.idempotency_key}, timeout=timeout)
    ...
    if attempt.provider == IntegrationConfig.Provider.PAYMOB:
        ...
        payload={"amount": int(attempt.amount * 100), "currency": attempt.currency, "payment_methods": cfg.config.get("payment_methods", []), "items": [], "billing_data": {"first_name":"FABINZI","last_name":"Customer","email":attempt.order.checkout.shipping_email or "NA","phone_number":attempt.order.checkout.shipping_phone or "NA"}, "extras": {"order_number": str(attempt.order.number)}}
...
def verify_stripe_signature(raw_body, signature_header, secret, tolerance=300):
    ...
    if abs(int(time.time())-timestamp)>tolerance: return False
...
def parse_webhook(provider, raw_body):
    ...
    if provider == "stripe":
        event_id=str(payload.get("id", "")); event_type=payload.get("type", ""); obj=payload.get("data",{}).get("object",{})
        reference=str(obj.get("id", "")); success=event_type=="payment_intent.succeeded"; failed=event_type in {"payment_intent.payment_failed","payment_intent.canceled"}
    else:
        obj=payload.get("obj", payload); event_id=str(payload.get("id") or obj.get("id") or hashlib.sha256(raw_body).hexdigest()); reference=str(obj.get("id") or obj.get("order",{}).get("id") or ""); success=bool(obj.get("success")); failed=bool(obj.get("pending") is False and not success)
